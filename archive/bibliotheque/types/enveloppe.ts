 ...
 * - une méthode pour calculer une valeur de hachage.
 *
 * Le type TIN peut être au format JSON ou non, être mutable ou non : s'il est mutable, il est confiné.
 * Ainsi, toute méthode ayant une occurrence positive de TIN est protected.
 * En effet, elle pourrait permettre d'obtenir l'état mutable et de réaliser un effet de bord
 * sur l'état s'il est mutable. C'est aux sous-classes de contrôler le confinement souhaité.
 * Le type TEX est : son usage n'est donc pas restreint. Il est utilisé
 * pour la sérialisation au format JSON.
 *
 * Cette classe abstraite doit être étendue. Pour l'extension, procéder ainsi :
 * - définir une interface pour un type de données, par dérivation de TypeEnveloppe,
 * - définir le type TIN pour représenter l'état, le type TEX pour le format de sérialisation et
 *   le type E pour les étiquettes utiles pour représenter ces données,
 * - définir une classe implémentant cette interface par héritage de Enveloppe,
 *   avec un constructeur appelant le constructeur parent en précisant la fonction de conversion de TIN vers TEX,
 * - implémenter les méthodes de l'interface et les méthodes abstraites de Enveloppe (net et représenter),
